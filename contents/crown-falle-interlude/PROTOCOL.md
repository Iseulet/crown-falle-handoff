...
- 브레인덤프 총 분량이 **500줄 초과** 시 강제 정리 세션 권고
  - **적용 범위: 엔티티 설정(캐릭터/장소/세력/규칙)만 카운트.** 서사 상세(장면 설계, 에피소드 구성, Beat 등)는 이 트리거에 포함하지 않는다. 서사 상세는 브레인덤프 소관이며, 분량이 늘어나는 것이 정상이다.
...
브레인덤프에서 다른 문서로 내용을 옮길 때의 공통 원칙. 캐논 분리와 세션 산출물 모두에 적용.
...
1. 원문의 내용을 **손실 없이** 옮긴다.
2. "정리"는 구조화(섹션 분리, 마커 추가, 항목 정렬)를 의미한다. 텍스트 축약이 아니다.
3. 설정의 맥락, 의도, 배경 설명, 다른 엔티티와의 관계 서술을 삭제하지 않는다.
4. 설계 방침(의도적 미설정 등)을 생략하지 않는다.

**구조화에서 허용하는 것:**
- 섹션 헤더 추가 (기본 정보, 성격, 관계, 서사 아크 등)
- [확정]/[미확정] 마커 부착
- 항목 순서 재배열
- 중복 문장 제거 (동일 정보가 브레인덤프에서 여러 곳에 흩어져 있던 경우)
- status 헤더, source 메타데이터 추가

**구조화에서 금지하는 것:**
- 원문의 서술을 짧게 요약하는 것
- 맥락 설명이나 의도 서술을 "불필요"로 판단해 삭제하는 것
- 관계 서술에서 한쪽 시점만 남기고 다른 시점을 삭제하는 것

#### 캐논 분리 적용

- 캐논 파일(profile.md, setting.md 등)은 집필(3단계)과 아트 진입의 소스. 밀도가 충분해야 한다.
- 브레인덤프 원문 → [확정]/[미확정] 마커 + 섹션 구조 추가 → 캐논 파일 완성. 내용 자체는 변경하지 않는다.

#### 세션 산출물 적용

- Desktop 세션 작업 문서(서사 상세, 에피소드 설계 등)는 **전체 내용을 브레인덤프에 삽입**한다.
- 각 이벤트의 상세 묘사, 캐릭터 내면, 분석, 서사 씨앗, 플레이어 선택 분기, 공간 지정, 항목 테이블 전부 포함.
- 작업 문서 원본은 `_braindump/session-drafts/`에 보관.
- 파일명: `braindump{NN}-{주제}.md`

### 브레인덤프 경량화 형식

분리 완료된 섹션은 **참조 링크 + 핵심 요약**으로 교체한다.

**요약 분량: 최소 3줄, 최대 5줄. 1줄 요약은 금지한다.**
...
Desktop에서 경량화된 브레인덤프만으로 전체 맥락을 잡을 수 있어야 한다.
...
이름: Tristram / 20대 중반 / 평민(상인 집안) / Armand의 에코르셔 연락책
성격: 냉소적, 관찰력, 거리를 둠. 관계: Wren(상호호감), Seren(점진적 호감→희생)
아크: 냉소 → 도구 → 암살(선 넘음) → 와해 → 복수 → 폭로/비극
최종 운명: [미확정]
```
...
```markdown
## Ironmead
> 분리 완료 → world/locations/ironmead/setting.md
> 상세는 setting.md 참조.

북서, 산맥 끝 + 서쪽 평원. Armand 영지. 변경 요새 + 곡창.
Tristram 출발지. 유민이 넘치는 도시.
```

**형식 예시 — 세력:**
```markdown
## 에코르셔
> 분리 완료 → world/factions/ecorcheurs.md
> 상세는 ecorcheurs.md 참조.

멸칭. 도적에 가까운 용병 집단 총칭. 통일 지휘 없음.
"전쟁보다 에코르셔가 무섭다." 그림메-에코 일부 사용.
새 왕 + 상비군 시 제거 대상.
```
...
| v4.5 | 2026-03-23 | 세션 산출물 처리 규칙 추가. session-drafts/ 보관, 브레인덤프 반영 시 축약 금지, narrative/ 캐논 분리 가능 밀도 유지 |
| v4.6 | 2026-03-23 | 내용 보존 원칙 통합(캐논 분리+세션 산출물 공통 규칙, 축약 금지/구조화만). 브레인덤프 경량화 형식 보강(3~5줄, 1줄 금지, 형식 예시 3종). 500줄 트리거 범위 명시(엔티티 설정만 카운트, 서사 상세 제외) |
