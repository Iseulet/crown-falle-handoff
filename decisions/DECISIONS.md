...
## [2026-03-13] ARM/RES 게이지형 전환
> 🇰🇷 [2026-03-13] ARM/RES 방어 시스템 게이지형 전환
> **Category:** combat | **Status:** ✅ 확정 | **Superseded By:** —

- **변경:** ARM/RES %형(CON×0.3, WIL×0.3) **폐기** → 게이지형(CON×arm_per_con, WIL×res_per_wil)으로 전환
  > 🇰🇷 기존 % 감소 방식 폐기. 피격 시 게이지 먼저 차감, 0이면 HP 직접 적용.
- **CC 게이트:** ARM > 0 → 물리 CC 면역 / RES > 0 → 마법 CC 면역
  > 🇰🇷 방어 게이지가 CC 발동의 게이트 역할. DOS2 방어 게이지 구조 채택.
- **config 변경:** `stat_derivation.con_arm_coefficient` / `wil_res_coefficient` 제거 → `armor.arm_per_con: 3` / `armor.res_per_wil: 3` 추가
  > 🇰🇷 combat_config.jsonc 구조 변경. armor 섹션 신설.
- **근거:** 단순하고 직관적. 방어 숫자 하나만 보면 됨. 점사 전략이 자연스럽게 유도됨.

---

## [2026-03-13] VP(사기) 시스템 도입
> 🇰🇷 [2026-03-13] VP(Valor Points, 사기) 시스템
> **Category:** combat | **Status:** ✅ 확정 | **Superseded By:** —

- **초기값:** 파티 인원 × 1 (4인 파티 = 4VP)
  > 🇰🇷 빡빡하게 시작. 모든 VP 사용이 전략적 결정.
- **파티 공유:** 개인 자원이 아닌 파티 전체 단일 풀
  > 🇰🇷 STA/MP는 개인 자원, VP는 파티 공유.
- **수급:** 적 처치 +1, 치명타 +1, 특정 직업 교전 성립 시 +1
- **소모:** 궁극기(2~3VP), VP 이탈(1VP), 전투 중 부상 회복
- **STA/MP/VP 삼원화:** 기존 이원화(STA/MP)에서 삼원화로 확장
  > 🇰🇷 자원 관리 긴장감 증대. 워테일즈 방식 채택.

---

## [2026-03-13] 원소 시스템 — 6종 3레이어
> 🇰🇷 [2026-03-13] 원소 6종, 3레이어 귀속 구조
> **Category:** combat | **Status:** ✅ 확정 | **Superseded By:** —

- **6종:** 독(Poison) / 화염(Fire) / 전기(Lightning) / 물(Water) / 빙결(Ice) / 어둠(Dark)
- **3레이어:**
  - 장비 레이어: 독 (무기 도포, 투척물)
  - 스킬 레이어: 화염/전기/물 (Mage 주력, 스킬 개별 속성)
  - 지형 레이어: 어둠 (전장 환경 종속, 시야 제한)
- **원소 귀속:** 캐릭터 적성 없음. 스킬과 아이템에 속성 부여.
  > 🇰🇷 적성 개념 폐기. 누구든 해당 스킬/아이템 보유 시 사용 가능.

---

## [2026-03-13] Mage 원소 적성 없음
> 🇰🇷 [2026-03-13] Mage 클래스 원소 적성 폐기
> **Category:** combat | **Status:** ✅ 확정 | **Superseded By:** —

- **결정:** Mage에 특정 원소 적성 없음. 모든 원소 스킬을 자유롭게 조합 가능.
- **INT 스케일링:** 원소 스킬 데미지는 INT × 계수로 계산
- **근거:** 원소는 캐릭터가 아닌 스킬에 귀속. 자유로운 빌드 구성 가능.

---

## [2026-03-13] 지형 효과 Tier 4 (후순위)
> 🇰🇷 [2026-03-13] 지형 표면 효과 구현 우선순위 결정
> **Category:** combat | **Status:** ✅ 확정 | **Superseded By:** —

- **결정:** 지형 표면 레이어(화염/독/얼음 표면, 원소 표면 상호작용)는 **Tier 4** 구현
- **어둠(Dark) 지형은 Tier 3** — 교전 조작 스킬과 연계되므로 Tier 3 포함
- **근거:** 전투 뼈대(스탯/공식/교전/스킬) 완성 후 얹어야 테스트 가능.

---

## [2026-03-13] 부상 시스템 Tier 3 도입
> 🇰🇷 [2026-03-13] 부상(Injury) 시스템 구현 시점 결정
> **Category:** combat | **Status:** ✅ 확정 | **Superseded By:** —

- **결정:** 부상 시스템은 **Tier 3** 구현 (전투 핵심 완성 후)
- **부상 발생 조건:** HP 25% 이하, ARM/RES=0 상태 큰 피해, CC 2회 이상, 전투 중 부활
